...
本チュートリアルで採用するアーキテクチャでは、既存のMVCパターンなどで使われるModel、Controllerとの概念の混同を避けるため、一部、独自の命名を採用しています（既存のModel、Controllerなどの役割は人によって解釈が異なるため、概念にゆらぎがないもののみ使用しています）。

<dl>
<dt>Exemplar／エグゼンプラ</dt><dd>原本や典型などの意。ここでは業務あるいはサービスを運用する上で必要なこと（＝要求）、知識、ロジックを持つオブジェクトを意味する。</dd>
<dt>Entity／エンティティ</dt><dd>実体の意。業務あるいはサービスで登場する概念が持つプロパティを保持するオブジェクト。システム開発で一般的に用いられるエンティティの概念と同じ。</dd>
