...
本チュートリアルで採用するアーキテクチャでは、既存のMVCパターンなどで使われるModel、Controllerとの概念の混同を避けるため、一部、独自の命名を採用しています。

<dl>
<dt>Exemplar／エグゼンプラ</dt><dd>原本や典型などの意。ここでは業務あるいはサービスを運用する上で必要なこと（＝要求）、知識、ロジックを持つオブジェクトを意味する。</dd>
<dt>Entity／エンティティ</dt><dd>業務あるいはサービスで登場する概念が持つプロパティを保持するオブジェクト。システム開発で一般的に用いられるエンティティの概念と同じ。</dd>
<dt>Usecase／ユースケース</dt><dd>オブジェクト指向プログラミングで一般的に使われるユースケースと等しい。ここではユースケースを、様々なScene／シーンの組み合わせによって複数のScenario／シナリオを取り得るものと定義します。</dd>
<dt>Actor／アクター</dt><dd>オブジェクト指向プログラミングで一般的に使われるアクターと等しい。アクターはユーザだけでなく、アプリ自身がアクターとしてユースケースを実行することもあります。</dd>
...
<dt>Dispatcher／ディスパッチャー</dt><dd>通信指令係の意。Fluxアーキテクチャで使われるDispatchの概念をそのまま輸入している。本アーキテクチャでは、ユーザによる操作時にユースケースの初めのシーン（initial scene）を引数にdispatchすることでユースケースの実行（＝パフォーマーの持つメソッドを呼び出し）するように実装します。</dd>
